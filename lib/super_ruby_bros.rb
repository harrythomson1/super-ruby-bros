...
@level_three_coins = LevelThreeCoins.new
...
def level_three_coin_collision1?
  if @level_three_coins.collision_coin(@player.x1, @player.y1, @player.x2, @player.y2, @player.x3, @player.y3, @player.x4, @player.y4)
    @level_three_coins.move_coin
    @player.coins += 1
  end
end

def level_three_coin_collision2?
  if @level_three_coins.collision_coin2(@player.x1, @player.y1, @player.x2, @player.y2, @player.x3, @player.y3, @player.x4, @player.y4)
    @level_three_coins.move_coin2
    @player.coins += 1
  end
end

def level_three_coin_collision3?
  if @level_three_coins.collision_coin3(@player.x1, @player.y1, @player.x2, @player.y2, @player.x3, @player.y3, @player.x4, @player.y4)
    @level_three_coins.move_coin3
    @player.coins += 1
  end
end
...
update do
  ...
  if @player.lives > 0 && level_one == true
    ...
    @level_three.draw
    @level_three_coins.draw
    @player.draw
    level_three_collision_detected?
    level_three_collision_detected_bottom?
    level_three_coin_collision1?
    level_three_coin_collision2?
    level_three_coin_collision3?
    player_methods
    @hero.x = @player.x - 5
    @hero.y = @player.y - 30
  else
